...
| Field | Value |
| --- | --- |
| Document version and date | 0.1 — 2026-09-21 |
...
| Requirements | 22 MUST / PROPOSED requirements; no RA-10 requirement accepted yet |
| Validation obligations | 16 proposed obligations for later verification; no product, model or user study executed |
| Policy decisions | Four recommendations awaiting Owner disposition |
| Static review | Author checks only; formal SR-RA10-001 not yet conducted |
...
**Shall** in this draft states a proposed obligation. It becomes binding only after the relevant content is ACCEPTED and baseline controls are complete. **MUST** is the proposed priority, not evidence of acceptance. Requirements addressed to the evaluation process may be fulfilled through controlled human work and later evaluation tooling; they do not automatically require new runtime features in TDG.
...
Verification must cover every rule ID, existing accepted target, the requirement-to-rule-to-VAL inverse view, and the boundary combinations required by VAL-014. References alone are not proof of adequate executable cases. The bridge is PROPOSED; OBS-001 remains open until its verification and disposition are accepted in the formal RA-10 review. The older RA-03 snapshot remains unchanged, and is not retrospectively claimed to have 32 direct REQ traces.

### 8.2 Phase-wide readiness and proportionality

The current source inventory contains 227 requirement rows and 153 validation-obligation rows across RA-01 through RA-09. This is an inventory count, not a statement that all relations, implementations or tests are complete. RA-10 proposes a further 22 requirements and 16 obligations. Earlier slices use direct references and controlled intermediate objects, so an aggregate direct-coverage percentage would be misleading without inspecting those chains.
...
## 9. Proposed requirements and forward trace

Every row is **MUST / PROPOSED**. `VAL-xxx` denotes `RA10-VAL-xxx` in Section 10. Supporting sections supply the detailed conditions; no new automatic evaluator, model judge, dashboard or database is required merely by this register.
...
These are proposed evidence obligations for static review, later test design and empirical work. They are not executed tests, a claim of independence, or one required executable test per requirement. `REQ-xxx` denotes `RA10-REQ-xxx`.
...
All four decisions below are OPEN. Accepting these recommendations would establish policies, not measured performance, selected numeric thresholds, or new runtime capabilities.
...
## 12. Author checks, remaining prerequisites, and exit

### 12.1 Author checks

The checks below concern this document and its references. They are performed by its author, not an independent reviewer, and are not a formal static-review result or product-test evidence.
...
| Local requirement identity | PASS — 22 unique MUST / PROPOSED rows; acceptance not implied by shall wording |
...
| Controlled inherited bridge | PASS for author check — all 11 existing IS rules link to existing accepted RA-03 requirements; 41 rule/requirement edges covering 23 distinct requirements, with inverse membership checked. Formal verification and OBS-001 disposition remain open. |
...
| Authority and scope | Author inspection: no runtime evaluator feature, model choice, automatic TC repair or protected-data authorization introduced; formal review still required |
...
| Overall Requirements Analysis exit | NOT COMPLETE — this draft and consolidated phase readiness remain to be reviewed |
...
The proposed RA03-VAL-014 bridge awaits formal verification and acceptance; the documentation-growth risk remains active. Phase-wide trace readiness, cross-slice consistency and an estimable scope/capacity view are required before a broad Requirements Analysis closure claim. Detailed executable test design and evidence remain STLC work; their absence at requirements review is not a fabricated PASS or an instruction to start implementation.

### 12.3 Exit and next action

The next action is the Owner's walkthrough of this draft, including the 22 proposed requirements, 16 validation obligations and four recommendations. After content disposition, conduct SR-RA10-001, record findings, prepare any corrections, and verify them. No final static-review verdict is preassigned.
