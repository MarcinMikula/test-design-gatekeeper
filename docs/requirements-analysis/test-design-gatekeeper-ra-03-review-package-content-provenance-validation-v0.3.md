...
| Workstream | `RA-03` |
| Status | BASELINE CANDIDATE — all RA-03 decisions dispositioned; upstream delta review, requirement acceptance, and focused RA-03 static review required |
| Decision authority | Project Owner |
| Entry authorization | `PG-RA02-001` — `GO`, 2026-09-09 |
| Approved upstream baselines | Project Charter v0.3; RA-01 v0.2; RA-02 v0.2 |
| Controlled upstream amendment candidates | Project Charter v0.4; RA-01 v0.3; RA-02 v0.3 |
| Implementation authorization | NOT GRANTED |
| Confidential-data use | NOT AUTHORIZED |

> **Document status notice:** This document specifies a serialization-neutral logical model and validation semantics for a Review Package. All ten RA-03 decisions were dispositioned by the Project Owner on 2026-09-12 and are incorporated in this v0.3 candidate. The fifty requirements remain `PROPOSED`: accepted decision directions do not by themselves baseline every requirement. Project Owner requirement acceptance, focused RA-03 static review, correction verification, and controlled verification of the three upstream amendment candidates remain required. This document does not select a JSON Schema, CSV layout, database schema, UI, API, storage engine, identifier algorithm, fingerprint algorithm, document parser, LLM, prompt, architecture, or deployment topology.
...
- `G` — a non-separable contradiction or integrity problem affects the complete basis;
- `D` — required conditional evidence exists for the requested assessment;
- `X` — an independent supported subset can be isolated.

`–` means the condition is not relevant because an earlier condition determines the outcome.
...
| IS-10 | Y | Y | Y | Y | Y | Y | Y | N | N | Y | Admit the package minimum; mark only dependent assessments insufficient or partial |
| IS-11 | Y | Y | Y | Y | Y | Y | Y | N | Y | Y | Permit qualification and the identified supported assessments; make no completeness or approval claim |

This decision table specifies semantic consequences. Exact status names, user messages, persistence events, and error codes belong to RA-05 and RA-09.
...
## 25. Review state and next action

Current state: `RA-03 BASELINE CANDIDATE — 10 OF 10 DECISIONS ACCEPTED; REQUIREMENTS AND STATIC REVIEW PENDING`.

Recommended walkthrough order:

1. Sections 1–6 — purpose, boundary, terminology, principles, and three-layer model;
2. Sections 7–13 — content, cardinality, scope, source, test-case, traceability, and provenance;
3. Sections 14–16 — conditional evidence, sufficiency decision table, identity, and lineage;
4. Section 17 — fifty candidate requirements;
5. Sections 18–19 — downstream tests and synthetic examples;
6. Section 20 — ten explicit decision dispositions;
7. Sections 21–24 — deferrals, quality check, exit criteria, and traceability.

This v0.3 consolidates every accepted RA-03 decision and is the candidate for the RA-03 baseline. Charter v0.4, RA-01 v0.3, and RA-02 v0.3 carry the controlled upstream wording changes while preserving the already effective historical phase gates. Before any recommendation or gate decision for `RA-04`, the package requires focused delta review of those upstream amendments, explicit disposition of the fifty proposed RA-03 requirements, focused RA-03 static review, and correction verification.

No transition to `RA-04`, solution design, repository creation, implementation, or confidential-data use is requested by this draft.
