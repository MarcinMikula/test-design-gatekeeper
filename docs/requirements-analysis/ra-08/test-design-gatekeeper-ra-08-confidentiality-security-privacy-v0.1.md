...
| Status | DRAFT — PROPOSED FOR OWNER REVIEW |
...
| Validation obligations | 16 proposed obligations; no product, deployment, or security tests executed |
| Policy decisions | 4 OPEN decisions, each with a recommendation |
| Formal static review | NOT PERFORMED — author checks only |
...
**Shall** describes a proposed obligation in this draft; it becomes binding for a requirement only when its status becomes **ACCEPTED** through an attributable Owner decision. **MUST** is the proposed priority, not evidence of acceptance. Sections 2–8 define the meaning and boundaries of the statements in Section 9. An accepted conditional requirement applies when its stated profile or feature exists; it does not require confidential-data operation in the laboratory.
...
Official OWASP material was consulted on 2026-09-20 for three limited reference points: instructions embedded in supplied documents can influence an LLM; access decisions need enforcement beyond an interface; and diagnostics can become a disclosure path. Project-specific controls below derive from TDG's accepted boundary and the proposed threat analysis, rather than wholesale adoption of a security checklist. [Prompt injection][owasp-injection] [Authorization][owasp-auth] [Logging][owasp-logging]
...
| Outbound handling | External evaluation must be deliberate and disclosed; a laboratory label cannot declassify a package | No protected bytes outside the approved boundary; proposed runtime policy in Section 5.3 denies all external egress |
...
| Intake condition | Proposed handling |
| --- | --- |
...
### 6.1 Protected copies — proposed OD-RA08-002
...
### 6.2 Diagnostics and authority history — proposed OD-RA08-003
...
### 6.3 Retention, deletion, and restoration — proposed OD-RA08-004
...
The sealed readiness record must identify the exact deployment/configuration, authorized data scope and users, approved policies, deployment boundary diagram and inventory, complete data flows and copy lifecycle, design-stage threat analysis, allocated controls, verification results and limitations, known residual risks, current validity conditions, and the attributable ROLE-09 decision. “Local,” a checked checkbox, this requirements draft, or an Owner SDLC gate is insufficient.
...
## 9. Proposed requirements and direct trace

All statements below have priority **MUST** and status **PROPOSED**. Section references are normative to their proposed meaning. `VAL-xxx` denotes `RA08-VAL-xxx`; no acceptance or executed validation is implied by a trace link.
...
These decisions refine policy choices not previously fixed. They do not reopen the already accepted prohibition on confidential laboratory processing, external protected-data use, or automatic business-rule invention. Each recommendation remains **PROPOSED** until accepted.
...
## 12. Author check, remaining allocations, and exit

### 12.1 Author check

This is a check by the same assistant that authored the draft. It is not the formal focused static review, independent security assurance, an executed control test, or Owner acceptance.
...
| Draft status and normative convention | PASS — all 24 MUST requirements are PROPOSED; “shall” acceptance rule explicit |
| Each proposed requirement has at least one direct validation obligation | PASS — 24/24 |
...
| Owner acceptance / formal review / confidential-data authorization | NOT YET ESTABLISHED for this slice or a deployment |
...
The next action is the Owner walkthrough of this draft, including Sections 9–11. Following the walkthrough, a focused static review will check the complete slice against the accepted baseline, resolve findings, verify corrections and traceability, and record the Owner's decisions. Original acceptances need not be requested twice; any later changes must be distinguishable.

Completion requires an accepted RA-08 baseline, resolved or explicitly dispositioned findings and decisions, and the attributable review record. Only the subsequent explicit gate may grant **GO to RA-09**. It does not authorize implementation, a particular deployment, or confidential-data use; those require their applicable later evidence and authority.

| Version | Date | Change |
| --- | --- | --- |
| 0.1 | 2026-09-20 | Initial proposed RA-08 slice following accepted RA-07 closure; 24 requirements, 16 validation obligations, and four recommended policy decisions |
